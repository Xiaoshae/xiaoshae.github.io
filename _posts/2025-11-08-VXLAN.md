...
+---------------------------------------------------------------+   VXLAN Header  |
|                VXLAN Network Identifier       |   Reserved_2  |  (UDP Playload) |
+---------------------------------------------------------------+------------------
```

![img](/assets/img/VXLAN/7b042303-a381-40c2-a092-6ed14ee07c35.png)
...
在 VXLAN 网络中，防止 BUM 流量在 VTEP 之间无限循环（形成广播风暴）的机制被称为**水平分割 (Split Horizon)**。

从 VXLAN 隧道（即覆盖网络）中接收到的数据包，在解封装后，只能被转发到本地的二层接口（例如连接的虚拟机或物理端口），**决不能再被转发到任何其他的 VXLAN 隧道（VXLAN VTEP）**中。
